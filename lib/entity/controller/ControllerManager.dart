...
import 'package:dart_hello_server/entity/controller/ControllerInfo.dart';

class ControllerManager {
  static var manager = ControllerManager();
//该list用于判断Controller是否已经被添加
  List<BaseController> controllers = [];
//这是一个map，对应的是请求链接，跟对应的controller信息
  Map<String, ControllerInfo> urlToMirror = {};
...
// 实际返回的Map
    var info = <String, ControllerInfo>{};
//请求地址对应的方法
    var urlToMethod = <String, Symbol>{};
// 获取Controller实例的镜子
    ...
    classMirror.declarations.forEach((symbol, declarationMirror) {
      ...
      if (symbol.toString() != classMirror.simpleName.toString()) {
        ...
        declarationMirror.metadata.forEach((medate) {
          //请求的地址
          var requestPath = path.join() + medate.reflectee.path;
          //请求的类型
...
//实例化一个Controller信息
    var controllerInfo = ControllerInfo(im, urlToMethod);
